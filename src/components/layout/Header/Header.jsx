...
export default function Header() {
  ...
  const spliting = useGSAP(()=>{
      if(split_heading_Ref.current){
        let split_heading = SplitText.create(split_heading_Ref.current,{type:"words,chars"});
      ...
      const spliting_prag = useGSAP(()=>{
        if(split_prag_Ref.current){
              let split_prag = SplitText.create(split_prag_Ref.current,{type:"words,chars"});
      ...
      <div className='absolute top-0 right-0 left-0 -z-10 h-screen overflow-hidden'>
        <div className='relative w-full h-full'>
          <div className='absolute *:text-center *:m-auto z-10 top-1/3 sm:top-1/2 left-1/2 transform -translate-x-1/2 text-white *:w-max-content'>
              <h1 ref={split_heading_Ref} className='text-second-color sm:text-2xl uppercase tracking-widest '>Luxury & Comfort</h1>
              <p ref={split_prag_Ref} className=' headingFont px-2 text-3xl sm:text-5xl uppercase'>Sophisticated Alpine <br/> Resort In</p>
          </div> 
            <Swiper
            modules={[Autoplay]}
            spaceBetween={50}
            ...
            onSlideChange={() => {spliting , spliting_prag}}
            // onSwiper={(swiper) => console.log(swiper)}
          >
            <SwiperSlide><img src={image1}  alt='hotel' className='object-cover w-full h-screen'/></SwiperSlide>
            <SwiperSlide><img src={image2}  alt='hotel' className='object-cover w-full h-screen'/></SwiperSlide>
            <SwiperSlide>
